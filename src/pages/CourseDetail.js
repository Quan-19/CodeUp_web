      ...
      <div className="course-banner">
        ...
        <div className="banner-content">
          ...
          <div className="course-meta">
            <span><Star size={16} fill="#ffc107" /> {course.rating || 4.5}</span>
            <span><Users size={16} /> {course.students || 0} học viên</span>
            <span><Clock size={16} /> {course.duration || "8 tuần"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="course-main">
              ...
              <div className="overview-tab">
                ...
                <ul className="learning-outcomes">
                  {course.learningOutcomes?.map((outcome, index) => (
                    ...
                    <>
                      <li><CheckCircle size={18} /> Kiến thức nền tảng vững chắc</li>
                      <li><CheckCircle size={18} /> Kỹ năng thực hành thực tế</li>
                      <li><CheckCircle size={18} /> Các case study từ doanh nghiệp</li>
                      <li><CheckCircle size={18} /> Chứng chỉ sau khi hoàn thành</li>
                    </>
                  )}
                </ul>
              </div>
                  ...
                  {course.curriculum?.map((chapter, index) => (
                    <div className="chapter" key={index}>
                      <div className="chapter-header">
                        <h4>Chương {index + 1}: {chapter.title}</h4>
                      ...
                      <ul className="lessons">
                        {chapter.lessons?.map((lesson, i) => (
                          <li key={i}>
                            ...
                            <span className="duration">{lesson.duration || "15 phút"}</span>
                          </li>
                        )) || (
                          <>
                            <li>
              ...
              <div className="instructor-tab">
                ...
                <div className="instructor-profile">
                  ...
                  <div className="instructor-info">
                    ...
                    <p className="title">{course.instructor?.title || "Senior Developer tại FPT Software"}</p>
                    <div className="instructor-meta">
                      <span><Star size={16} fill="#ffc107" /> {course.instructor?.rating || 4.8}</span>
                      <span><Users size={16} /> {course.instructor?.students || 1250} học viên</span>
                    ...
                    <div className="stars">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star 
                          key={star} 
                          size={20} 
                          fill={star <= Math.round(course.rating || 4.5) ? "#ffc107" : "#e0e0e0"} 
                        />
                      ))}
                    </div>
                    <span className="count">{course.reviews?.length || 0} đánh giá</span>
                  </div>
                    ...
                    {[5, 4, 3, 2, 1].map((rating) => (
                      <div className="rating-bar" key={rating}>
                        ...
                        <div className="bar-container">
                          <div 
                            className="bar" 
                            style={{ width: `${(course.ratingDistribution?.[rating] || 0) * 100}%` }}
                          ></div>
                        </div>
                        <span className="percentage">{
                          Math.round((course.ratingDistribution?.[rating] || 0) * 100)
                        }%</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {course.reviews?.length > 0 ? (
                    course.reviews.map((review, index) => (
                      <div className="review" key={index}>
                        ...
                        <span className="review-date">{review.date || "2 tuần trước"}</span>
                      </div>
                    ))
                  ) : (
        ...
        <div className="course-sidebar">
          <div className="pricing-card">
            <div className="price">
              <span className="current-price">{course.price?.toLocaleString() || "1,990,000"} VND</span>
              {course.originalPrice && (
                <span className="original-price">{course.originalPrice.toLocaleString()} VND</span>
              )}
            </div>
